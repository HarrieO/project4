...
public class Board {
    ...
    public Board( int min, String state){
        this.height = 6;
        this.width = 6;
    ...
    private Car readCar(String carState){
        ...
        next = carState.indexOf(',');
        x = Integer.valueOf(carState.substring(start, next));
        start = next+1;
        next = Math.max(carState.indexOf("H", start),
                        carState.indexOf("V", start));
        y = Integer.valueOf(carState.substring(start,next));
        horizontal = carState.charAt(next) == 'H';
        start = next+1;
        length = Integer.valueOf(carState.substring(start));
        return new Car(x,y,length,horizontal) ;
    }
    ...
    private void setState(String state){
        ...
        next = state.indexOf(';');
        if(next > -1) {
            addExitCar(readCar(state.substring(start, next)));
            start = next + 1;
            next = state.indexOf(';',start);
            while (next > -1) {
                addCar(readCar(state.substring(start, next)));
                start = next + 1;
                next = state.indexOf(';',start);
            }
        }
